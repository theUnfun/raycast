...
Player::Player(int x, int y, const map::Map& map)
	: position_(x, y)
	, projection_distance_(0.5 * WALL_HEIGHT / std::tan(pi / 180 * (0.5 * FOV_VERTICAL)))
	, map_(map) {
...
void Player::Draw(sf::RenderWindow& window_2d, sf::RenderWindow& window_3d) {
	...
	for (const auto& distance : distances_) {
		...
		float height = (WINDOW_HEIGHT * projection_distance / distance);
		...
		if (std::abs(distance - VISION_RANGE) > EPSILON) {
			...
			rect.setFillColor(sf::Color(255 * (1 - distance / VISION_RANGE), 255 * (1 - distance / VISION_RANGE), 255 * (1 - distance / VISION_RANGE)/*, brightness*/));
			rect.setOrigin(0, -WINDOW_HEIGHT / 2 + height / 2);
...
double Player::GetDistanceBetweenObjects(const sf::Vector2f& position1, const sf::Vector2f& position2) {
...
void Player::DrawVisionRay(sf::RenderWindow& window_2d) const {
	...
	vision_ray[1].position = sf::Vector2f(position_.x - VISION_RANGE * std::cos(pi / 180 * (direction_ + 90.)), position_.y - VISION_RANGE * std::sin(pi / 180 * (direction_ + 90)));
...
void Player::ComputeDistances() {
	...
	for (int i = 0; i < NUMBER_OF_RAYS; ++i) {
		float pixel_pos = half_far_plane_width * 2. * (float(i) + 0.5 - NUMBER_OF_RAYS / 2.) / float(NUMBER_OF_RAYS);
		...
		ray[1].position = sf::Vector2f(position_.x - max_ray_length * std::cos(pi / 180 * (angle + direction_ + 90.)),
			position_.y - max_ray_length * std::sin(pi / 180 * (angle + direction_ + 90.)));
...
void Player::MovePlayer(float deltatime) {
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) {
		step.x += MOVEMENT_SPEED * cos(pi / 180 * (direction_ + 90.));
		step.y += MOVEMENT_SPEED * std::sin(pi / 180 * (direction_ + 90.));
	}
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) {
		step.x += -MOVEMENT_SPEED * std::cos(pi / 180 * (direction_ + 90.));
		step.y += -MOVEMENT_SPEED * std::sin(pi / 180 * (direction_ + 90.));
	}
