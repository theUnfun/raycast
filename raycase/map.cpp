...
namespace map {
	...
	void Map::Update(const sf::Event& event) {
		...
		if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left && !is_drawing) {
			start_position_ = sf::Vector2f(event.mouseButton.x, event.mouseButton.y);
		...
		if (event.type == sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left && is_drawing) {
			end_position_ = sf::Vector2f(event.mouseButton.x, event.mouseButton.y);
	...
	std::vector<sf::Vector2f> Map::GetCollisionsForRay(const Line& ray) const {
